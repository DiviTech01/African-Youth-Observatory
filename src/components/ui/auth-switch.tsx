...
export function AuthSwitch({ onSignIn, onSignUp, className, initialMode = 'signin' }: AuthSwitchProps) {
  ...
  return (
    <div
      className={cn(
        "relative w-full max-w-[820px] mx-auto rounded-2xl overflow-hidden border border-gray-800 shadow-2xl shadow-black/50",
        "sm:min-h-[520px]",
        className
      )}
    >
      {/* Forms Container */}
      <div className="flex w-full sm:min-h-[520px]">
        {/* ── Mobile (single column, only one form visible) ──────────────── */}
        <div className="sm:hidden w-full p-5">
          {/* Header that doubles as the toggle */}
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {isSignUp ? "Join AYO for free" : "Welcome back to AYO"}
              </p>
            </div>
            <button
              type="button"
              onClick={() => setIsSignUp(!isSignUp)}
              className="text-xs font-semibold text-[#D4A017] hover:underline shrink-0"
            >
            ...
            <form onSubmit={handleSignIn} className="space-y-3">
              ...
              <div className="relative">
                <input
                  ...
                  required
                  className="w-full h-10 px-3 pr-12 rounded-lg bg-white/[0.05] border border-gray-800 text-white placeholder:text-gray-500 text-sm focus:outline-none focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017] transition-colors"
                />
        ...
        {/* ── Desktop (split-pane, both forms with sliding overlay) ──────── */}
        {/* Sign In Form - Left Side */}
        <div
          className={cn(
            "hidden sm:flex w-1/2 p-6 md:p-8 flex-col justify-center transition-all duration-500 ease-in-out",
      ...
      <motion.div
        className="hidden sm:block absolute top-0 w-1/2 h-full z-20"
        initial={false}
